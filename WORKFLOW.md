...
tracker:
  kind: linear
  project_slug: "lindy-orchestrator-e01c9328908a"
  active_states:
    - Todo
    - In Progress
    - Merging
...
polling:
  interval_ms: 5000
workspace:
  root: ~/code/lindy-orchestrator-workspaces
hooks:
  after_create: |
    git clone --depth 1 git@github.com:eddieran/lindy-orchestrator.git .
    ./scripts/preflight-github-transport.sh || echo "[preflight] WARNING: no working GitHub transport — fetch/push/PR workflows will fail"
...
This is **lindy-orchestrator** -- a lightweight, git-native multi-agent orchestration framework for autonomous project execution. It acts as the harness engineer of a codebase: decomposing goals into dependency-ordered task DAGs, dispatching AI coding agents to isolated module directories, validating output through pluggable QA gates, and coordinating the pipeline via markdown and git.

- Source repo: git@github.com:eddieran/lindy-orchestrator.git
- Language: Python 3.11+
- Package manager: uv
- Build: `uv sync --extra dev --frozen`
- Test: `pytest tests/ -x -q --tb=short`
- Lint: `ruff check src/ tests/`
- Format: `ruff format --check src/ tests/`
- CLI entry: `lindy-orchestrate` (via `src/lindy_orchestrator/cli.py`)

## Prerequisite: Linear access is available

The agent must be able to talk to Linear via one of: Linear MCP server, injected `linear_graphql` tool, or direct GraphQL API calls. If none are available, stop and record the blocker in the workpad.
...
- `debug`: investigate test failures and execution issues.
...
- When meaningful out-of-scope improvements are discovered during execution, file a separate Linear issue in the same project instead of expanding scope. Never create issues for environment, sandbox, tooling, or infrastructure problems — record those as blockers in the workpad and stop.
...
- **Linear access**: read/update issues, post comments (via MCP, `linear_graphql`, or GraphQL API).
- **Git operations**: branch, commit, push, pull, merge — use standard git commands.
- **GitHub CLI**: `gh pr create`, `gh pr merge`, `gh pr checks` — for PR management.
...
1. Find or create a single persistent workpad comment for the issue:
   ...
   - Ignore resolved comments while searching; only active/unresolved comments are eligible.
...
Before pushing or moving to Human Review:
...
2. Gather feedback from all channels:
   ...
   - Inline review comments (`gh api repos/eddieran/lindy-orchestrator/pulls/<pr>/comments`).
...
- Do NOT create Linear issues for sandbox, tooling, network, or permission problems — record in workpad and stop.
