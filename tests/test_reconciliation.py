...
def test_skipped_event_emitted_when_dep_fails(mock_provider: MagicMock) -> None:
    ...
    plan = TaskPlan(
        goal="test",
        tasks=[
            TaskSpec(id=1, module="root", description="first", status=TaskStatus.FAILED),
            TaskSpec(
...
def test_cascading_skip_a_b_c(mock_provider: MagicMock) -> None:
    ...
    plan = TaskPlan(
        goal="cascade",
        tasks=[
            TaskSpec(id=1, module="root", description="A", status=TaskStatus.FAILED),
            TaskSpec(id=2, module="root", description="B", depends_on=[1]),
            TaskSpec(id=3, module="root", description="C", depends_on=[2]),
        ],
    )
...
def test_independent_task_not_skipped(mock_provider: MagicMock) -> None:
    ...
    plan = TaskPlan(
        goal="partial",
        tasks=[
            TaskSpec(id=1, module="root", description="fails", status=TaskStatus.FAILED),
            TaskSpec(
                id=2,
                module="root",
                description="blocked",
            ...
            TaskSpec(id=3, module="root", description="independent", status=TaskStatus.PENDING),
        ],
    )
