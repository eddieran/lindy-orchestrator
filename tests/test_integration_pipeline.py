...
class TestPlannerGeneratorEvaluatorPipeline:
    ...
    def test_execute_plan_uses_distinct_worktrees_per_task(
        ...
        working_dirs = {Path(call["working_dir"]) for call in provider.dispatch_calls}
        assert working_dirs == set(worktrees)
    ...
    def test_execute_plan_parallel_ready_tasks_overlap_with_barrier(
        ...
        def side_effect(call: dict[str, object]) -> DispatchResult:
            ...
            return DispatchResult(
                module=str(call["module"]), success=True, output="done", event_count=1
            )

        mock_create_provider.return_value = RecordingProvider(side_effect=side_effect)
    ...
    @patch("lindy_orchestrator.orchestrator.create_provider")
    def test_dispatch_loop_retries_after_retryable_eval_failure(
        self, mock_create_provider, mock_run_qa_gate, tmp_path: Path
    ) -> None:
    ...
    @patch("lindy_orchestrator.orchestrator.run_qa_gate")
    @patch("lindy_orchestrator.orchestrator.create_provider")
    def test_dispatch_loop_stops_after_max_retries(
        self, mock_create_provider, mock_run_qa_gate, tmp_path: Path
    ) -> None:
        from lindy_orchestrator.orchestrator import _dispatch_loop
        ...
        mock_create_provider.return_value = RecordingProvider()
        mock_run_qa_gate.side_effect = [
            QAResult(gate="command_check", passed=False, output="FAILED one"),
            QAResult(gate="command_check", passed=False, output="FAILED two"),
        ]
    ...
    @patch("lindy_orchestrator.orchestrator.run_qa_gate")
    @patch("lindy_orchestrator.orchestrator.create_provider")
    def test_dispatch_loop_skips_retry_for_non_retryable_eval_failure(
        self, mock_create_provider, mock_run_qa_gate, tmp_path: Path
    ) -> None:
        from lindy_orchestrator.orchestrator import _dispatch_loop
        ...
        mock_create_provider.return_value = RecordingProvider()
        mock_run_qa_gate.return_value = QAResult(
            gate="command_check",
            passed=False,
    ...
    def test_dispatch_loop_passes_eval_feedback_back_into_generator_prompt(
        ...
        retry_prompt = str(provider.dispatch_calls[1]["prompt"])
        assert "Previous attempt failed QA verification" in retry_prompt
        assert "tests/test_api.py" in retry_prompt
    ...
    def test_pipeline_can_use_distinct_provider_instances_per_role(
        ...
        mock_generator_create_provider.side_effect = [generator_provider, generator_provider]
        mock_evaluator_create_provider.return_value = evaluator_provider

        plan = generate_plan("Ship auth", cfg)
        plan.tasks[0].skip_gates = ["structural_check"]
