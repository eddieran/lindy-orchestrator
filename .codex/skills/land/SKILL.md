...
# Only wait for Codex review if a Codex review workflow exists:
if ls .github/workflows/*codex* .github/workflows/*review* 2>/dev/null | rg -q '.'; then
  while true; do
...
- If all jobs fail on the merge commit with dependency issues, fetch latest
...
- Do not enable auto-merge unless the repo has required status checks configured.
...
- If Codex review is configured, reviews arrive as issue comments posted by
  GitHub Actions starting with `## Codex Review — <persona>`. If present,
  treat them as feedback that must be acknowledged before merge. If no Codex
  review workflow exists in `.github/workflows/`, skip this wait path.
