...
class MetricsCollector:
    ...
    def _get_or_create_task(self, event: Event) -> TaskMetrics:
        """Return existing TaskMetrics or create a minimal one. Must be called under lock."""
        task_id = event.task_id
        assert task_id is not None  # callers guard on this
        tm = self._tasks.get(task_id)
        if tm is None:
            tm = TaskMetrics(task_id=task_id, module=event.module)
            self._tasks[task_id] = tm
        return tm
    ...
    def _dispatch(self, event: Event) -> None:
        ...
        match event.type:
            ...
            case EventType.TASK_COMPLETED | EventType.TASK_FAILED:
                if event.task_id is not None:
                    tm = self._get_or_create_task(event)
                    tm.status = "completed" if event.type == EventType.TASK_COMPLETED else "failed"
